...
export class UserRegistrationService {
 ...
 // Making the api call for the user registration endpoint
  ...
  public getUserMovies(name : String): Observable<any> {
    const token = localStorage.getItem('token');
    return this.http.get(apiUrl + 'users' + name, {headers: new HttpHeaders(
      {
        Authorization: 'Bearer ' + token,
  ...
  public addUserMovie(name : String, title : String): Observable<any> {
    const token = localStorage.getItem('token');
    return this.http.get(apiUrl + 'users' + name + '/movie' + title, {headers: new HttpHeaders(
      {
        Authorization: 'Bearer ' + token,
      })}).pipe(
      map(this.extractResponseData),
  ...
  public editUser(name : String): Observable<any> {
    const token = localStorage.getItem('token');
    return this.http.get(apiUrl + 'users' + name, {headers: new HttpHeaders(
      {
        Authorization: 'Bearer ' + token,
  ...
  public deleteUserMovie(name : String, title : String): Observable<any> {
    const token = localStorage.getItem('token');
    return this.http.get(apiUrl + 'users' + name + '/movie' + title, {headers: new HttpHeaders(
      {
        Authorization: 'Bearer ' + token,
